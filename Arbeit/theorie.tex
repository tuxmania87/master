...
Der erste und naheliegenste Ansatz ist es, beide Ergebnismengen iterativ zu vergleichen. Wir gehen also beide Mengen Schritt für Schritt durch. Treffen wir dabei auf unterschiedliche Elemente, so geben wir an in welchem Tupel sich beide Mengen unterscheiden und beenden den Vergleich. Bei dieser Vorgehensweise eröffnen sich mehrere Probleme. 

Ein Hauptproblem ist die Sortierung. Dabei spielt sowohl die Reihenfolge der Spalten, als auch die Sortierung der Tupel eine Rolle. Ist in der Aufgabenstellung gefordert, dass Spalten A, B und C ausgegeben werden sollen, so ist nicht unbedingt klar, ob bei der Ausgabe erst Spalte A, dann B und dann C ausgegeben werden muss, oder ob eine beliebige Permuation nicht ebenso zulässig ist. Hier müsste mein in der Aufgabe deutlich machen, ob die Spalten in einer gewissen Reihenfolge auszugeben sind. Ist dies der Fall muss der Dozent im Programm ebenfalls mit einer Option anschalten, dass die Spaltenreihenfolge gleich sein muss. Ist dies nicht der Fall, so muss das Programm prüfen ob alle Spalten in den, zu vergleichenden Tupeln, enthalten sind (und nicht mehr).

Das zweite Problem ist die Reihenfolge der Tupel. Enthält die Musterlösung ein \verb|ORDER BY|-Statement, so gehen wir davon aus, dass die Tupel in einer bestimmten Reihenfolge vom Lernenden ausgegeben werden müssen. Ist dies nicht der Fall, so müssen wir wieder alle Ergebnistupel der beiden Anfragen miteinander abgleichen. 

Eine einfache Umsetzung der Abgleichung von unsortierten Tupeln und Spalten ist eine künstliche Sortierung. Haben wir kein \verb|ORDER BY|-Statement in der Musterlösung und der Dozent hat per Option vermerkt, dass die Spaltenreihenfolge ebenfalls egal ist, dann sortieren wir zunächst die Spaltennamen lexikographisch. Anschließend Sortieren wir die Tupel sukzessive nach der ersten, zweiten, dritten, usw. Spalte aufsteigend. Nun können wir iterativ alle Tupel durchgehen und paarweise vergleichen.

Ein weiteres Problem bilden Spaltenüberschriten. Auch hier müsste der Dozent vermerken, ob Spaltenüberschriften eine Rolle spielen oder nicht. Notfalls kann sich das Programm an der Musterlösung orientieren. Werden in dieser Spaltenüberschriften verwendet, so könnten wir davon ausgehen, dass diese gefordert sind. Es wäre allerdings sinnvoller, die Notwendigkeit einer Spaltenüberschrift per Option an-/ abschaltbar zu machen, da unterschiedliche Dozenten die Übungsdatenbank befüllen könnten und diese unterschiedliche Stile bei der Musterlösung haben.

Für den Lernenden reicht es nicht aus die Tupel zu erwähnen, die beim Vergleich von seiner Anfrage mit der Musterlösung fehlen bzw. überflüssig sind. Diese Aussage wäre nicht sehr informativ und zielführend. Stattdessen muss das Programm sowohl die Datensätze der Musterlösung, als auch die Datensätze der Anfrage des Lerneden tabellarisch ausgeben. Durch unsere, eingangs erwähnte, Sortierung der Ergebnismengen, entstehen zwei Tabellen, die übersichtlich begreiflich machen, welche Tupel dem Lerneden Fehlen, oder welche überflüssig sind. Wir sehen uns dazu Abbildung \ref{fig:screenshot_tables} an.

\subsection{Behandeln von Fehlern}

- Syntaxfehler unterschiedlicher Natur, verweis auf abschnitt oben
- inkonsistente Bedingungen, evt im 1. schritt erkannt
- Diskussion: Standardisierte oder studentenanfrage einlesen
- Laufzeitfehler behandeln/melden
