...
\hline
$r\in \textit{Relation}$ & OR & $\le$ & $\ge$ & $>$ & $<$ & $=$ & IS NULL & IS NOT NULL  \\\hline
...
Die Abbildung \ref{fig:pesudocode1} soll den bisherigen Prozess darstellen. Dabei sortiert die Methode \verb|sortiere_menge| die eingegebene Menge gemäß der Ordnung in Abbildung \ref{fig:sortorder}.
...
\end{verbatim}
\caption{Pseudocode: Erster Teil der Sortierung}
...
Wir fahren dann fort und sortieren die Kindknoten $c_1,c_2$ und $c_3$ rekursiv. Am Ende erhalten wir den Ausdruck \verb| (b=2 OR c IS NULL) AND (a<6 OR d=5) AND a<5|, welcher durch den Baum in Abbildung \ref{fig:sort_exfin1} dargestellt wird.
...
Haben wir also den Fall, dass zwei Kindknoten $c_i,c_j$ von $T(r)$ den gleichen Operator repräsentieren müssen wir rekursiv die beiden Teilbäume $T(c_i)$ und $T(c_j)$ untersuchen. Wir führen dabei eine gleichzeitge und schrittweise Tiefensuche in beiden Bäumen durch, indem wir sukzessive einen Schritt einer Tiefensuche in einem Baum, dann im anderen Baum durchführen und schließlich einen nächsten Schritt, in gleicher Weise durchführen. Wir bezeichen die Knoten, bei denen sich die Tiefensuche in einem aktuellen Moment befindet als $v_i$ für $T(c_i)$ und $v_j$ für $T(c_j)$. Sobald in einem Schritt $v_i \neq v_j$ ist, entscheiden wir anhand von $\mathit{order}(v_i)$ und $\mathit{order}(v_j)$, welcher Teilbaum vor dem anderen erscheinen soll. $T(c_i)$ erscheint damit vor $T(c_j)$, wenn $\mathit{order}(v_i) < \mathit{order}(v_j)$. Dabei schließen wir Blattknoten von der Betrachtung aus, es sind also nur innere Knoten von Bedeutung.
